...
<div class='panel'>
  <table class='table-list'>
    <thead>
      <tr>
        <th width='50' class='center'>X</th>
        <th width='50' class='center'>Y</th>
        <th width='80' class='center'>寬度</th>
        <th width='80' class='center'>高度</th>
        <th width='100' class='left'>事件類型</th>
        <th class='left'>參數</th>
        <th width='90'>編輯</th>
      </tr>
    </thead>
...
<?php foreach ($objs as $obj) { ?>
        <tr>
          <td class='center'><?php echo $obj->x;?></td>
          <td class='center'><?php echo $obj->y;?></td>
          <td class='center'><?php echo $obj->width;?></td>
          <td class='center'><?php echo $obj->height;?></td>
          <td class='left'><?php echo RichmenuAction::$actionTypeNames[$obj->action_type];?></td>
          <td class='left'>
      <?php if ($obj->action_type == RichmenuAction::ACTION_TYPE_1) { ?>
              <div>訊息：<?php echo $obj->text;?></div>
      <?php }
            if ($obj->action_type == RichmenuAction::ACTION_TYPE_2) { ?>
              <div>網址：<?php echo $obj->uri;?></div>
      <?php }
            if ($obj->action_type == RichmenuAction::ACTION_TYPE_3) { ?>
              <div>回傳資料：<?php echo $obj->data;?></div>
      <?php }
            if ($obj->action_type == RichmenuAction::ACTION_TYPE_4) { ?>
              <div>時間類型：<?php echo RichmenuAction::$actionPickTypeNames[$obj->action_pick_type];?></div>
      <?php } ?>
          </td>

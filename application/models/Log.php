...
class Log extends OaModel {
  ...
  public static function getEventInstanceof ($event, &$callback) {
    ...
    if ($event instanceof ImageMessage) { $callback = function ($event, $log, &$obj) { $params = array ('log_id' => $log->id, 'reply_token' => $event->getReplyToken () ? $event->getReplyToken () : '', 'message_id' => $event->getType () == 'message' ? $event->getMessageId () : '', 'file' => ''); return LogImage::transaction (function () use (&$obj, &$log, $params) { return verifyCreateOrm ($obj = LogImage::create (array_intersect_key ($params, LogImage::table ()->columns))) && $log->setStatus (Log::STATUS_CONTENT); }); }; return 'ImageMessage'; };
    if ($event instanceof VideoMessage) { $callback = function ($event, $log, &$obj) { $params = array ('log_id' => $log->id, 'reply_token' => $event->getReplyToken () ? $event->getReplyToken () : '', 'message_id' => $event->getType () == 'message' ? $event->getMessageId () : '', 'file' => ''); return LogVideo::transaction (function () use (&$obj, &$log, $params) { return verifyCreateOrm ($obj = LogVideo::create (array_intersect_key ($params, LogVideo::table ()->columns))) && $log->setStatus (Log::STATUS_CONTENT); }); }; return 'VideoMessage'; };
    if ($event instanceof AudioMessage) { $callback = function ($event, $log, &$obj) { $params = array ('log_id' => $log->id, 'reply_token' => $event->getReplyToken () ? $event->getReplyToken () : '', 'message_id' => $event->getType () == 'message' ? $event->getMessageId () : '', 'file' => ''); return LogAudio::transaction (function () use (&$obj, &$log, $params) { return verifyCreateOrm ($obj = LogAudio::create (array_intersect_key ($params, LogAudio::table ()->columns))) && $log->setStatus (Log::STATUS_CONTENT); }); }; return 'AudioMessage'; };

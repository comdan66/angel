...
class Log extends OaLineModel {
  ...
  public static function getEventInstanceof ($event, &$callback) {
    ...
    if ($event instanceof VideoMessage) { $callback = function ($event, $log, &$obj) { $params = array ('log_id' => $log->id, 'reply_token' => $event->getReplyToken () ? $event->getReplyToken () : '', 'message_id' => $event->getType () == 'message' ? $event->getMessageId () : ''); return LogVideo::transaction (function () use (&$obj, &$log, $params) { return verifyCreateOrm ($obj = LogVideo::create (array_intersect_key ($params, LogVideo::table ()->columns))) && $log->setStatus (Log::STATUS_CONTENT); }); }; return 'VideoMessage'; };
    if ($event instanceof AudioMessage) { $callback = function ($event, $log, &$obj) { $params = array ('log_id' => $log->id, 'reply_token' => $event->getReplyToken () ? $event->getReplyToken () : '', 'message_id' => $event->getType () == 'message' ? $event->getMessageId () : ''); return LogAudio::transaction (function () use (&$obj, &$log, $params) { return verifyCreateOrm ($obj = LogAudio::create (array_intersect_key ($params, LogAudio::table ()->columns))) && $log->setStatus (Log::STATUS_CONTENT); }); }; return 'AudioMessage'; };
